...
import { normalizeComment, normalizeQuote, normalizeScreenName } from '../util'
import { getMyProfileStats } from '../api'
import { VK_GROUP_ID } from '../config'
import MyProfileGraphs from '../components/MyProfileGraphs'
import MyProfileLastTopics from '../components/MyProfileLastTopics'
import MyProfileBotQuotes from '../components/MyProfileBotQuotes'
...
const MyProfile = () => {
  ...
  useEffect(() => {
    setLoading(true)
    getMyProfileStats().then(result => {
      setUser(result.user)
      setUserStats(result.stats)
      setTopicWithMoreComments(result.topic_with_more_comments)
      setCommentsWithMoreLikes(result.comments_with_more_likes)
      setTopicWithMoreLikes(result.topic_with_more_likes)
    }).catch(err => {
    ...
    <div>
      <Spin tip="Carregando..." spinning={loading}>
        ...
        <Tabs type="card">
          ...
          <TabPane tab="Citações Bot" key="3">
            <MyProfileBotQuotes/>
          </TabPane>
          <TabPane tab="Últimos tópicos" key="4">
            <MyProfileLastTopics/>
          </TabPane>
        </Tabs>
      </Spin>
